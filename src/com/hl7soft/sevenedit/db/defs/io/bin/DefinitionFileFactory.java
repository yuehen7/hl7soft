...
public class DefinitionFileFactory implements IDefinitionFactory {
	public static final int VERSION_3_0 = 1;
	public static final int VERSION_3_1 = 2;
	int formatVersion = 2;
	File file;
	DefinitionFactory cache = new DefinitionFactory();
	ZipFile zipFile;
	IFormatDescriptor descriptor;

	public DefinitionFileFactory(File file) throws FileNotFoundException, UnknownDefinitionFileFormat {
		this.file = file;
		initialize();
	}

	private void initialize() throws FileNotFoundException, UnknownDefinitionFileFormat {
		try {
			if (!this.file.exists()) {
	...
	public MessageDefinition getMessageDefinition(String version, String name) {
		MessageDefinition messageDefinition = this.cache.getMessageDefinition(version, name);
		if (messageDefinition == null) {
			try {
				String ref = this.descriptor.getMessageRef(version, name);
				ZipEntry entry = this.zipFile.getEntry(ref);
				if (entry != null) {
					messageDefinition = readMessageDefinition(decryptionFilter(this.zipFile.getInputStream(entry)));
					this.cache.addMessageDefinition(messageDefinition);
					messageDefinition.setFactory(this);
				}
			} catch (Exception e) {
				throw new RuntimeException("Error loading message definition: " + name + ", version" + version, e);
			}
		}
	...
	public boolean isMessageDefinitionCached(String version, String name) {
		return this.cache.getMessageDefinition(version, name) != null;
	}

	public SegmentDefinition getSegmentDefinition(String version, String name) {
		SegmentDefinition segmentDefinition = this.cache.getSegmentDefinition(version, name);
		if (segmentDefinition == null) {
			try {
				String ref = this.descriptor.getSegmentRef(version, name);
				ZipEntry entry = this.zipFile.getEntry(ref);
				if (entry != null) {
					segmentDefinition = readSegmentDefinition(decryptionFilter(this.zipFile.getInputStream(entry)));
					this.cache.addSegmentDefinition(segmentDefinition);
					segmentDefinition.setFactory(this);
				}
			} catch (Exception e) {
				throw new RuntimeException("Error loading segment definition: " + name + ", version" + version, e);
			}
		}
	...
	public boolean isSegmentDefinitionCached(String version, String name) {
		return this.cache.getSegmentDefinition(version, name) != null;
	}

	public FieldDefinition getFieldDefinition(String version, String name) {
		FieldDefinition fieldDefinition = this.cache.getFieldDefinition(version, name);
		if (fieldDefinition == null) {
			try {
				String ref = this.descriptor.getFieldRef(version, name);
				ZipEntry segmentEntry = this.zipFile.getEntry(ref);
				if (segmentEntry != null) {
					fieldDefinition = readFieldDefinition(decryptionFilter(this.zipFile.getInputStream(segmentEntry)));
					this.cache.addFieldDefinition(fieldDefinition);
					fieldDefinition.setFactory(this);
				}
			} catch (Exception e) {
				throw new RuntimeException("Error loading field definition: " + name + ", version" + version, e);
			}
		}
	...
	public boolean isFieldDefinitionCached(String version, String name) {
		return this.cache.getFieldDefinition(version, name) != null;
	}
	...
	public void flushResources() {
		this.cache = new DefinitionFactory();
	}

	public String getMessageDescription(String version, String name) {
		return this.descriptor.getMessageDescription(version, name);
	}

	public String getSegmentDescription(String version, String name) {
		return this.descriptor.getSegmentDescription(version, name);
	}

	public String getFieldDescription(String version, String name) {
		return this.descriptor.getFieldDescription(version, name);
	}
	...
	class XmlDescriptorWrapper implements DefinitionFileFactory.IFormatDescriptor {
	...
	class LegacyFormatDescriptorWrapper implements DefinitionFileFactory.IFormatDescriptor {
		...
		public String getMessageDescription(String version, String name) {
			MessageDefinition def = DefinitionFileFactory.this.getMessageDefinition(version, name);
		...
		public String getSegmentDescription(String version, String name) {
			SegmentDefinition def = DefinitionFileFactory.this.getSegmentDefinition(version, name);
			return def != null ? def.getDescription() : null;
		}
		...
		public String getFieldDescription(String version, String name) {
			FieldDefinition def = DefinitionFileFactory.this.getFieldDefinition(version, name);
			return def != null ? def.getDescription() : null;
		}
